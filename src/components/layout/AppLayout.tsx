...
const navItems: Record<UserProfile, Array<{ label: string; icon: React.ElementType; path: string }>> = {
  mulher: [
    { label: 'Início', icon: Home, path: '/mulher' },
  ...
  profissional: [
    { label: 'Início', icon: Home, path: '/profissional' },
  ...
  gestora: [
    ...
    { label: 'Relatórios', icon: BarChart3, path: '/gestora/relatorios' },
            ...
            <div>
              {title ? (
                ...
                <>
                  ...
                  <h1 className="text-base font-semibold text-foreground">Olá, {currentUser.nome.split(' ')[0]}</h1>
                </>
              )}
            </div>
          </div>
